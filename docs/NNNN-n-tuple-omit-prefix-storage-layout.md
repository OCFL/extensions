...
4. Starting at the leftmost character of the resulting id and working right, divide the id into `numberOfTuples` each containing `tupleSize` characters. It is expected that this resulting UTF-8 identifier is split between grapheme clusters (e.g. character sequences such as (g + °),  digraphs such as Slovak “ch”, etc. - see more at https://www.unicode.org/reports/tr29/tr29-1.html#Grapheme_Cluster_Boundaries).
