...
| `size`                | The size of the binary file in bytes, expressed as an integer with no padding. For example, the `size` digest of a zero-length bitstream is `0` (>= 1 digit long). |
